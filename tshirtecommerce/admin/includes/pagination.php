...
class Zebra_Pagination {
    ...
    private $_properties = array(
        ...
        // customized by phinq
        'tshirtecommerce'           =>  false,

        // customized by phinq
        'tshirtecommerce_type'      => '',
     ...
     *  @return void
     */
    public function render($return_output = false, $tshirtecommerce = false, $type = '') {
    ...
    private function _build_uri($page) {
        ...
        if ($this->_properties['method'] == 'url') {
            ...
            /*if (strpos($this->_properties['base_url_query'], '/') !== false) {
        ...
        // if page propagation is to be done through GET
            ...
            // compatibility with tshirtecommerce
            if ($this->_properties['tshirtecommerce'] === true && $this->_properties['tshirtecommerce_type'] == 'clipart') {

                $str_query_clipart = urldecode(http_build_query($query));
                $str_query_clipart = str_replace('/clipart/ajax/', '', $str_query_clipart);
                $str_query_clipart = str_replace('&page=', '', $str_query_clipart);
                $str_array = explode('=', $str_query_clipart);

                $new_query = isset($str_array[0]) ? $str_array[0] : '';

                $new_array = array();
                if (!empty($new_query)) {
                    $new_array = explode('/', $new_query);
                }
                $id_category = ('clipart' == $new_array[1]) ? 0 : $new_array[1];
                $new_query = $this->_properties['tshirtecommerce_url'].'/index.php?/clipart/index/'.$page.'/'.$id_category;

                $new_query .= '&page='.$page; // Fixed paging
    ...
    // customized by phinq
    public function tshirtecommerce($tshirtecommerce = false)
    {
        $this->_properties['tshirtecommerce'] = $tshirtecommerce;
    }

    // customized by phinq
    public function tshirtecommerce_type($tshirtecommerce_type = '')
    {
        $this->_properties['tshirtecommerce_type'] = $tshirtecommerce_type;
    }

    public function tshirtecommerce_url($tshirtecommerce_url = '')
    {
        $this->_properties['tshirtecommerce_url'] = $tshirtecommerce_url;
    }
