...
var app = {
	admin:{
		...
		add: function(e)
		{
			...
			if (jQuery(e).hasClass('design-idea') == true) {
				...
				if (id.indexOf(':') == -1) {
					...
					jQuery('#_product_id').attr('value',id);
					document.getElementById('_product_title_img').value = title +'::'+ img;
					jQuery('#_product_title_img').attr('value',title +'::'+ img);
		...
		save: function(data, type){
			...
			jQuery('#product_form_save_go_to_catalog_btn').click();
			return true;
		}
	},
	cart: function(content){
		...
		//if(parseInt(data.id_combination) && data.id_combination != null) {
			var _data = {
				...
				'id_product_attribute': data.id_combination,
				'action': 'update',
				'id_customization': 0,
				//'group[1]': 1,
				//'group[3]': 8
		...
		jQuery.ajax({
			...
			url: baseUri + 'index.php?controller=cart',
			type: 'POST',
			async: true,
			cache: false,
			data: _data,
			...
			success: function (json) {
				if(json.hasError == true) {
					...
					if (typeof window.parent.ps_link_shopping_cart !== 'undefined') {
						window.location.href = window.parent.ps_link_shopping_cart.replace('&amp;', '&');
					} else {
...
function setupPrice(e)
{
	...
	jQuery('#_product_title_img').attr('value',img);
}
...
function resizePageDesign(e){
	...
	if (check) {
		...
		var height = jQuery('#add_designer_product').height() + 20;
