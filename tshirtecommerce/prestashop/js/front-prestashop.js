...
var prestashop = {
    datas_tmp: {},
    selectColor: function(e) { /* @since 1.0.6 */
        jQuery('#color_to_pick_list > li').removeClass('selected');
        jQuery(e).parent().addClass('selected');
        jQuery('#ps_color_pick_hidden').val(jQuery(e).data('id'));
...
design.ajax.getPrice = function() {
    ...
    jQuery.ajax({
        ...
        success: function(json) {
            ...
            // datas.ps_specific = json.specific; // @deprecated 1.3.0.b
...
design.products.addProduct = function(products) {
    ...
    jQuery.each(products, function(i, product){
        var div = document.createElement('div');
            ...
            if (typeof product.parent_id !== 'undefined') {
                div.setAttribute('ps-product-id', product.parent_id);
            }
            div.className = 'product-box col-xs-6 col-sm-4 col-md-3';
        jQuery(div).click(function(){ 
            //seft.changeProduct(this, product);
            design.products.changeDesign(div, product.id);
        });         
...
design.products.productCate = function(id) {
    ...
    if (typeof seft.products[id] != "undefined") {
        ...
        var cat_idx = 1, cat_idx_id = 1, category_id = 0;
        
        while (jQuery("#parent-categories-"+cat_idx).length > 0) {
            cat_idx_id = cat_idx;
            if (jQuery("#parent-categories-"+cat_idx).val() == 0) {
                var tmp = cat_idx - 1;
                category_id = jQuery("#parent-categories-"+tmp).val();
            } else {
                category_id = jQuery("#parent-categories-"+cat_idx).val();
            }
            cat_idx++;
        }
        if (category_id == 'undefined' || category_id == undefined) category_id = 0;
        jQuery.ajax({
            type: "POST",
            dataType: "json",
...
jQuery(document).on('ini.design', function(event) {
	if (typeof window.parent.tshirtecommerce_admin === "undefined" && window.parent.tshirtecommerce_admin != 1) {
        if (typeof window.parent.baseDir !== 'undefined') {
            $jd.ajax({
                url: window.parent.baseDir + "modules/tshirtecommerce/ajax.php",
                type: "POST",
                data: {
    ...
    if (typeof min_order !== 'undefined') {
        if (input_size.length > 0) jQuery(input_size[0]).val(min_order);
        jQuery('#quantity').val(min_order);
    }

    // re-calc height on designer tool
    var width_site = window.parent.getfullWidth();
    if (width_site > 500) {
        var wh = jQuery('#dg-wapper').height() + 30;
        jQuery('#dg-wapper').css('height', wh+'px');
    }
...
jQuery(document).on('after.addtocart.design', function(event, data) {
	if (data != "") {
	    ...
	    if (content.error == 0) {
            ...
            content.product.id_combination = prestashop.datas_tmp.id_product_attribute;
	        content.product.design_imgs = "";
            content.product.attributes = jQuery('#tool_cart').serializeObject();
            content.product.tshirtecommerce_color = jQuery('#product-list-colors .bg-colors.active').data('color');

            if (typeof product_id !== 'undefined') {
                content.product.tshirtecommerce_product_id = product_id;
            } else {
                content.product.tshirtecommerce_product_id = 0;
            }
