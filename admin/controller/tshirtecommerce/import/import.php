...
<div class="media-frame wp-core-ui mode-grid mode-edit">
	<div class="media-frame-content" data-columns="7">
		<ul style="padding: 0px;" class="attachments">
			
			<?php foreach($products as $product) { ?>
			<li style="margin: 10px;float: left;list-style: none;" class="attachment save-ready">
	...
	if(typeof div[0] != 'undefined')
	{
		...
		jQuery.post(ajaxurl, data, function(response) {
			if(response != 0)
			{
				...
				if(response.indexOf('9file.net') == -1)
					response = '<?php echo $base_url; ?>image/'+response;
