...
class ModelvendorProduct extends Model {
	public function addProduct($data) {
		...
		if(isset($mailinfo['status'])){
			...
			$replace = array(
				...
				'model' 	=> $data['model'],
				'emails' 	=> $emails,
				'loginlink' => $this->url->link('vendor/product', '', 'SSL') . "\n\n"
			);
	...
	public  function getMailInfo($sellertype){
		$query=$this->db->query("select * from " . DB_PREFIX . "vendor_mail vm LEFT JOIN " . DB_PREFIX . "vendor_mail_language vml on(vm.mail_id=vml.mail_id) where vm.sellertype='" .$sellertype."' and vml.language_id = '" . (int)$this->config->get('config_language_id') . "'");
		 //echo "select * from " . DB_PREFIX . "vendor_mail vm LEFT JOIN " . DB_PREFIX . "vendor_mail_language vml on(vm.mail_id=vml.mail_id) where vm.sellertype='" .$sellertype."' and vml.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		//die();
		return $query->row;
	...
	public function getProducts($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "vendor_to_product v2p LEFT JOIN " . DB_PREFIX . "product p ON (v2p.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (v2p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and v2p.vendor_id<>0";
		...
		if (isset($data['filter_vendor'])) {
			$sql .= " AND v2p.vendor_id LIKE '" . $this->db->escape($data['filter_vendor']) . "%'";
		}
