...
class ControllerVendorquestions extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected'])) {
			...
			$this->response->redirect($this->url->link('vendor/questions',$url, true));
		}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if($result['answered']==0){
			  ...
			  $product=$this->model_vendor_product->getProduct($result['product_id']);
			...
			if (!$result['approved']) {
				$approve = $this->url->link('vendor/questions/approve','&user_question_id=' . $result['user_question_id'] . $url, true);
			} else {
				$approve = '';
			}
			
			if ($result['approved']) {
				$disapproved = $this->url->link('vendor/questions/disapprove','&user_question_id=' . $result['user_question_id'] . $url, true);
			} else {
				$disapproved = '';
			}
			...
			$productvendorid= $this->model_vendor_questions->getAnswersVendorName($product['product_id']);
			
			$sellers = $this->model_vendor_vendor->getVendor($productvendorid['vendor_id']);
			...
			$data['questions'][] = array(
                ...
                'view'              => $this->url->link('vendor/questions/info','&user_question_id=' . $result['user_question_id']. $url, true),
		...
		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('vendor/questions',$url, true),
      		'separator' => ' :: '
   		);
		


		$data['delete'] = $this->url->link('vendor/questions/delete',$url, true);
        
        $data['setting'] = $this->url->link('vendor/questions/setting',$url, true);
		...
		$data['sort_image'] = $this->url->link('vendor/questions','&sort=image' . $url, true);
		$data['sort_productname'] = $this->url->link('vendor/questions','&sort=pd.name' . $url, true);
		$data['sort_name'] = $this->url->link('vendor/questions','&sort=uq.name' . $url, true);
		$data['sort_email'] = $this->url->link('vendor/questions', '&sort=uq.email' . $url, true);
		$data['sort_question'] = $this->url->link('vendor/questions','&sort=uq.question' . $url, true);
		$data['sort_date_added'] = $this->url->link('vendor/questions','&sort=uq.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('vendor/questions',$url . '&page={page}', true);
    ...
    public function info() {
        ...
        $data['action'] = $this->url->link('vendor/questions/add','&user_question_id=' . $user_question_id.  $url, true);
		$data['cancel'] = $this->url->link('vendor/questions',true);
    ...
    public function setting() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->response->redirect($this->url->link('vendor/questions',true));
		}
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('vendor/questions',true)
		);

		$data['action'] = $this->url->link('vendor/questions/setting',true);
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$url = '';
			$this->response->redirect($this->url->link('vendor/questions/info','&user_question_id=' . $this->request->get['user_question_id']. $url, true));
		}
		$this->info();
	}
