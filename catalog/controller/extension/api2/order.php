...
class ControllerExtensionApi2Order extends Controller {
	public function add() {
		...
		foreach ($products as $product) {
			...
			if ($product['minimum'] > $product_total) {
				...
				$json['product'] = array('name' => $product['name']);
				break;
			}
		}
		...
		if(version_compare(VERSION, '3.0') >= 0){
			...
			foreach ($results as $result) {
				if ($this->config->get('total_' . $result['code'] . '_status')) {
					...
					$this->{'model_extension_total_' . $result['code']}->getTotal($order_data['totals']);
				}
			}
