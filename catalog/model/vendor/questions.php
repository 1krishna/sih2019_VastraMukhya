...
class ModelVendorquestions extends Model {
	public function addAnswers($user_question_id, $data) {
		...
		name='".$this->vendor->getFirstName()."',
		email='".$this->vendor->getEmail()."',
		user_question_id='". $user_question_id."',
		date_added = now()";
		$this->db->query($sql);
		 //email 
		$userquestionid=$this->model_vendor_questions->getAnswersSingal($user_question_id);
		if(!empty($userquestionid['email'])){
			$emailuser=$userquestionid['email'];
		}
		if(!empty($userquestionid['question'])){
			$question=$userquestionid['question'];
		}
		if(!empty($userquestionid['name'])){
			$name=$userquestionid['name'];
		}
		...
		$message .= 'Q: '.$question."\n\n";
		$message .= 'A: '.$this->db->escape($data['answer'])."\n\n";
		...
		$mail->setSubject(html_entity_decode(sprintf($this->language->get('subject'), $emailuser), ENT_QUOTES, 'UTF-8'));
	...
	public function getquestions($data) {
		/* update */
		$sql = "SELECT *,uq.name as username FROM " . DB_PREFIX . "user_questions uq LEFT JOIN " . DB_PREFIX . "product_description pd ON (uq.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "vendor_to_product vtp ON (vtp.product_id = pd.product_id)  where vendor_id=vtp.vendor_id and uq.user_question_id<>0 AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		/* update */
	...
	public function getTotalquestions($data) {
		//$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "user_questions");
		/* update */
	...
	public function addReply($user_question_id,$data){
		...
		if($query->row){
			...
			$message .= 'Q: '.$query->row['question'].'<br /><br />';
			...
			$mail->setSender($this->config->get('config_name'));
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('subject'), $query->row['email']), ENT_QUOTES, 'UTF-8'));
			$mail->setText(strip_tags($message));
			$mail->send();
	...
	public function getAnswers($user_question_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user_questions_answer where user_question_id='".$user_question_id."' ORDER BY date_added DESC");		
		return $query->rows;
	}
	...
	public function getAnswersVendorName ($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "vendor_to_product where product_id='".$product_id."'");		
		return $query->row;
	}

	public function getAnswersProduct($product_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "vendor_to_product where product_id='".$product_id."'");		
		return $query->row;
	}
